...
class BattleGraphics(object):
    ...
    def draw(self, surface):
        ...
        self.player_box = pygame.Rect(padding, HEIGHT - h - padding, GAME_WIDTH - padding * 2, h)
        menu_box = self.player_box.inflate(-self.player_box.width // 1.3, -padding * 2)
        menu_box.topleft = (self.player_box.width - menu_box.width - padding, self.player_box.y + padding)

        message_box = self.player_box.inflate(-self.player_box.width // 1.7, -self.player_box.height // 2)
        message_box.topleft = (self.player_box.center[0] - message_box.width // 2, self.player_box.bottom - message_box.height - padding)
        ...
        health_box = pygame.Rect(padding * 4, self.player_box.y + padding * 2, w, BLOCK_SIZE // 2)
        ...
        pygame.draw.rect(surface, (200, 0, 200), self.player_box)
        ...
        hp_ratio = self.player_bo.hp / self.player_bo.max_hp
        ...
        mp_ratio = self.player_bo.mp / self.player_bo.max_mp
        ...
        hp_text = font.render(f"HP:{self.player_bo.hp} / {self.player_bo.max_hp}", True, (0, 0, 0))
        surface.blit(hp_text, health_box.inflate(-text_padding * 2, -text_padding))

        mp_text = font.render(f"MP:{self.player_bo.mp} / {self.player_bo.max_mp}", True, (0, 0, 0))
